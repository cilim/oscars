  ...
  class WinnersController < BaseController
      ...
      if @winner.save
        respond_to do |format|
          ...
          format.turbo_stream do
            @season_category   = SeasonCategory.includes(:category, nominees: :movie, winner: { nominee: :movie }, pick_selections: { player: :user, pick_type: :scoring_scheme }).find(@winner.season_category_id)
    ...
    def destroy
      ...
      respond_to do |format|
        ...
        format.turbo_stream do
          @season_category   = SeasonCategory.includes(:category, nominees: :movie, winner: { nominee: :movie }, pick_selections: { player: :user, pick_type: :scoring_scheme }).find(@season_category_id)
          @season_categories = @season.season_categories.includes(:winner)
          @scoreboard_data   = ScoreboardCalculator.new(@season).call
        end
      end
